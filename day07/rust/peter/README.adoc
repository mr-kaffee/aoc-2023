...
The most tricky part is to determine the type of the hand. The algorithm (implemented in `HandType::from(&[u8; 5])`) is as follows: count the number of occurrences for every card type in a hand (into a hash set), sort the number of occurrences (neglecting the card type) and choose the hand type based on the two biggest number of occurrences (for part 1, the number of jokers is always 0).
